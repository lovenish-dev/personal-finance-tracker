...
} from "./transaction.respository.js";
import type {
...
export async function createTransactionService(
  ...
  try {
    ...
    const accountOwnership = await getAccountForTransaction(
      client,
      data.accountId,
      userId
    );
    if (!accountOwnership) throw new AppError("Account Not Found", 404);
    const categoryOwnership = await getCategoryForTransactions(
      client,
      data.categoryId,
      userId
    );
...
export async function getTransactionsByUserIdService(
  userId: number,
...
export async function getTransactionByUserIdService(
  id: number,
  userId: number
) {
...
export async function deleteTransactionService(userId: number, id: number) {
  ...
  try {
    ...
    const reversingBalance = await reverseAccountBalance(
      client,
      deletedTransaction,
      userId
    );
    if (!reversingBalance)
      throw new AppError("Balance could not be reversed", 500);
    await client.query("COMMIT");
...
export async function updateTransactionService(
  ...
  try {
    ...
    const reversedBalance = await reverseAccountBalance(
      client,
      oldTransaction,
      userId
    );
    ...
    const newTransaction = {
      ...
      transactionDate: data.transactionDate ?? oldTransaction.transaction_date,
    };

    const updateBalance = await updateAccountBalance(client, userId, {
    ...
    const updatedTransaction = await modifyTransaction(client, id, userId, {
      accountId: newTransaction.accountId,
      categoryId: newTransaction.categoryId,
      amount: newTransaction.amount,
      type: newTransaction.type,
    ...
    throw new AppError("Transaction update failed", 500);
    await client.query("COMMIT");
