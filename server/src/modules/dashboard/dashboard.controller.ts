...
import { getCategorySummaryService, getDashboardSummaryService, getMonthlySummaryService } from "./dashboard.service.js";
...
export async function getMonthlySummary(req: AuthRequest, res: Response, next: NextFunction){
   try{
      if(!req.userId) throw new AppError("Id not Found", 401);
      const result = await getMonthlySummaryService(req.userId);
      res.status(200).json({ success: true, message:"Monthly summary fetched successfully",data: result })
   }catch(err){
      next(err)
   }
}